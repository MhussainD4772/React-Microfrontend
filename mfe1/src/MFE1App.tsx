...
export default function MFE1App({ appId, version, initialBulbOn, initialTargets, eventBus }: MFE1AppProps) {
  const [bulbOn, setBulbOn] = useState(initialBulbOn ?? false)
  const [selectedTargets, setSelectedTargets] = useState<string[]>(initialTargets ?? ['mfe1'])
  ...
  useEffect(() => {
    ...
    const unsub = eventBus.on('BULB_STATE_CHANGED', (msg: unknown) => {
      ...
      if (m?.targets?.includes('mfe1')) setBulbOn(!!m.state)
    })
    return () => unsub()
  }, [eventBus])
  ...
  return (
    <div className="w-full flex justify-center p-3 min-h-[280px]">
      <div className="w-full max-w-md rounded-2xl border border-slate-200/80 bg-white/95 backdrop-blur shadow-xl shadow-slate-200/50 p-6 animate-fade-in-up hover:shadow-2xl hover:shadow-violet-100/50 hover:-translate-y-0.5 transition-all duration-300">
        <h2 className="text-xs font-bold uppercase tracking-widest text-violet-600 mb-4">MFE1</h2>
        ...
        <button
          ...
          className="w-full py-3.5 px-4 rounded-xl font-semibold text-white bg-gradient-to-r from-violet-500 to-violet-600 hover:from-violet-600 hover:to-violet-700 active:scale-[0.98] shadow-lg shadow-violet-200 hover:shadow-violet-300/60 transition-all duration-200 mb-5"
        >
          Toggle Targets
        </button>
